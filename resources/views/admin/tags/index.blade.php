...
<div class="flex justify-between items-center mb-8">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage the tags used across your blog posts.</p>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-500">#{{ $tag->id }}</td>
                        <td class="px-6 py-4 font-medium text-gray-800">{{ $tag->name }}</td>
                        <td class="px-6 py-4 text-gray-500">{{ $tag->slug }}</td>
                        <td class="px-6 py-4 text-right">
                            <button type="button" onclick="openEditModal({{ $tag->id }}, '{{ addslashes($tag->name) }}', '{{ addslashes($tag->slug) }}')" class="text-indigo-600 hover:text-indigo-800 transition-colors mx-1"><i class="fas fa-edit"></i></button>
                            <button class="text-red-500 hover:text-red-700 transition-colors mx-1"><i class="fas fa-trash"></i></button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-500">No tags found. Create one to get started!</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
            <h3 class="font-bold text-gray-800 border-b border-gray-100 pb-3 mb-4">Add New Tag</h3>
            ...
            <form action="{{ route('admin.tags.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Laravel" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                    @error('name')
            ...
            <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Edit Tag</h3>
                        <div class="mt-4">
